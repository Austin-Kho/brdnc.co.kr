...
class Daily_money_report extends CB_Controller {
	...
	public function download(){
		...
		//----------------------------------------------------------//
        // 회사 정보
        $company = $this->input->get('com_sel');

		// 자금일보 출력 일자
		$ref_date = $this->input->get('ref_date', TRUE);
		$d_obj = date_create($ref_date);
		...
		//은행계좌 데이터
		$bank_acc = $this->cms_m4_model->select_data_lt('cb_cms_capital_bank_account', '', "no=1 OR com_seq={$company}", '');

		// 은행 계좌별 전일 잔고 및 금일 출납, 잔고 구하기 데이터
		for($i=0; $i<$bank_acc['num']; $i++) {
			$cum_in[$i] = $this->cms_main_model->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class2!=7) AND in_acc='".$bank_acc['result'][$i]->no."' AND deal_date<='".$ref_date."' ");
			$date_in[$i] = $this->cms_main_model->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class2!=7) AND in_acc='".$bank_acc['result'][$i]->no."' AND deal_date ='".$ref_date."' ");
			$cum_ex[$i] = $this->cms_main_model->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND out_acc='".$bank_acc['result'][$i]->no."' AND deal_date<='".$ref_date."' ");
			$date_ex[$i] = $this->cms_main_model->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND out_acc='".$bank_acc['result'][$i]->no."' AND deal_date ='".$ref_date."' ");
		}

		// 회사 현금자산 설정일 전일잔고 및 금일 출납, 잔고 구하기 데이터
		$cum_inc = $this->cms_main_model->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class2!=7) AND deal_date<='".$ref_date."' ");
		$date_inc = $this->cms_main_model->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class1!=3 AND class2!=7) AND deal_date ='".$ref_date."' ");
		$date_exp = $this->cms_main_model->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND class1!=3 AND deal_date ='".$ref_date."' ");
		$cum_exp = $this->cms_main_model->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND deal_date<='".$ref_date."' ");
		...
		// 조합 대여금 데이터
		$jh_data = $this->cms_m4_model->select_data_lt('cb_cms_capital_cash_book', 'any_jh', "com_seq={$company} AND any_jh<>0", 'any_jh');
		for($i=0; $i<$jh_data['num']; $i++){
			...
			$jh_cum_in[$i] = $this->cms_main_model->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class2!=7) AND is_jh_loan='1' AND any_jh = '".$jh_data['result'][$i]->any_jh."' AND deal_date<='".$ref_date."' "); //총 회수금
			$jh_date_in[$i] = $this->cms_main_model->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class2!=7) AND is_jh_loan='1' AND any_jh = '".$jh_data['result'][$i]->any_jh."' AND deal_date='".$ref_date."' "); // 당일 회수
			$jh_cum_ex[$i] = $this->cms_main_model->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND is_jh_loan='1' AND any_jh ='".$jh_data['result'][$i]->any_jh."' AND deal_date<='".$ref_date."' "); // 총 대여금
			$jh_date_ex[$i] = $this->cms_main_model->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND is_jh_loan='1' AND any_jh = '".$jh_data['result'][$i]->any_jh."' AND deal_date='".$ref_date."' "); // 당일 대여
		}

		// 조합 대여금 자산 설정일 전일잔고 및 금일 출납, 잔고 구하기 데이터
		$jh_cum_inc = $this->cms_main_model->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class2!=7) AND is_jh_loan='1' AND deal_date<='".$ref_date."' "); //총 회수금
		$jh_date_inc = $this->cms_main_model->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0 AND class2!=7) AND is_jh_loan='1' AND deal_date='".$ref_date."' "); // 당일 회수
		$jh_cum_exp = $this->cms_main_model->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND is_jh_loan='1' AND deal_date<='".$ref_date."' "); // 총 대여금
		$jh_date_exp = $this->cms_main_model->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE com_seq={$company} AND (com_div>0) AND is_jh_loan='1' AND deal_date='".$ref_date."' "); // 당일 대여
		...
		// 설정일 입금 내역
		$da_in = $this->cms_m4_model->select_data_lt("cb_cms_capital_cash_book", "account, cont, acc, inc, note", " com_seq={$company} AND (com_div>0 AND class2<>8) AND (class1='1' or class1='3') AND deal_date='".$ref_date."'", "", "seq_num");
		// 설정일까지 입금 내역
		$da_in_total = $this->cms_m4_model->da_in_total('cb_cms_capital_cash_book', $company, $ref_date);
		// 설정일 출금내역
		$da_ex = $this->cms_m4_model->select_data_lt("cb_cms_capital_cash_book", "account, cont, acc, exp, note", " com_seq={$company} AND (com_div>0) AND (class1='2' or class1='3') AND deal_date='".$ref_date."'", "", "seq_num");
		// 설정일까지 출금내역
		$da_ex_total = $this->cms_m4_model->da_ex_total('cb_cms_capital_cash_book', $company, $ref_date);
		//----------------------------------------------------------//
		/** 데이터 가져오기 종료 **/
    ...
    $spreadsheet->getProperties()->setCreator(site_url())
      ...
      ->setDescription('일별 자금 입출금 보고서');
		...
		// 워크시트 이름 지정
		$spreadsheet->getActiveSheet()->setTitle('자금일보('.$ref_date.')');
