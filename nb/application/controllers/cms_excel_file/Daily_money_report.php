...
class Daily_money_report extends CI_Controller {
	...
	public function excel_file(){
		...
		for($i=0; $i<$bank_acc['num']; $i++) {
			$cum_in[$i] = $this->cmain_m->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND in_acc='".$bank_acc['result'][$i]->no."' AND deal_date<='".$sh_date."' ");
			$date_in[$i] = $this->cmain_m->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND in_acc='".$bank_acc['result'][$i]->no."' AND deal_date ='".$sh_date."' ");
			$cum_ex[$i] = $this->cmain_m->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND out_acc='".$bank_acc['result'][$i]->no."' AND deal_date<='".$sh_date."' ");
			$date_ex[$i] = $this->cmain_m->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND out_acc='".$bank_acc['result'][$i]->no."' AND deal_date ='".$sh_date."' ");
		}

		// 회사 현금자산 설정일 전일잔고 및 금일 출납, 잔고 구하기 데이터
		$cum_inc = $this->cmain_m->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND deal_date<='".$sh_date."' ");
		$date_inc = $this->cmain_m->sql_result("SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND deal_date ='".$sh_date."' ");
		$date_exp = $this->cmain_m->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND deal_date ='".$sh_date."' ");
		$cum_exp = $this->cmain_m->sql_result("SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND deal_date<='".$sh_date."' ");
		...
		for($i=0; $i<$jh_data['num']; $i++){
			$jh_name[$i] = $this->cmain_m->sql_result(" SELECT pj_name FROM cb_cms_project WHERE seq = '".$jh_data['result'][$i]->any_jh."' ORDER BY seq ");//조합명
			$jh_cum_in[$i] = $this->cmain_m->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND is_jh_loan='1' AND any_jh = '".$jh_data['result'][$i]->any_jh."' AND deal_date<='".$sh_date."' "); //총 회수금
			$jh_date_in[$i] = $this->cmain_m->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND is_jh_loan='1' AND any_jh = '".$jh_data['result'][$i]->any_jh."' AND deal_date='".$sh_date."' "); // 당일 회수
			$jh_cum_ex[$i] = $this->cmain_m->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND is_jh_loan='1' AND any_jh ='".$jh_data['result'][$i]->any_jh."' AND deal_date<='".$sh_date."' "); // 총 대여금
			$jh_date_ex[$i] = $this->cmain_m->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND is_jh_loan='1' AND any_jh = '".$jh_data['result'][$i]->any_jh."' AND deal_date='".$sh_date."' "); // 당일 대여
		}

		// 조합 대여금 자산 설정일 전일잔고 및 금일 출납, 잔고 구하기 데이터
		$jh_cum_inc = $this->cmain_m->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND is_jh_loan='1' AND deal_date<='".$sh_date."' "); //총 회수금
		$jh_date_inc = $this->cmain_m->sql_result(" SELECT SUM(inc) AS inc FROM cb_cms_capital_cash_book WHERE (com_div>0 AND class2!=7) AND is_jh_loan='1' AND deal_date='".$sh_date."' "); // 당일 회수
		$jh_cum_exp = $this->cmain_m->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND is_jh_loan='1' AND deal_date<='".$sh_date."' "); // 총 대여금
		$jh_date_exp = $this->cmain_m->sql_result(" SELECT SUM(exp) AS exp FROM cb_cms_capital_cash_book WHERE (com_div>0) AND is_jh_loan='1' AND deal_date='".$sh_date."' "); // 당일 대여
