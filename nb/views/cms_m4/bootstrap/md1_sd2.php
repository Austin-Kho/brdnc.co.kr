        ...
        <div class="col-md-12">
            <div class="row bo-top bo-bottom" style="margin: 0 0 20px 0;">
                ...
                <div class="col-xs-5 col-md-2" style="padding: 11px 0px; text-align: right;">
                    ...
                    <button type="button" class="close" aria-label="Close" style="padding-left: 2px;" onclick="document.getElementById('s_date').value=''; document.getElementById('e_date').value='';">
                ...
                <table class="table table-bordered table-hover table-condensed font12">
                    <thead>
                        <tr style="border-top: 1px solid #ddd; background-color: #EAEAEA;">
                            ...
                            <th style="width: 110px;" class="center">
                                ...
                                <a href="javascript:" onclick="popUp_size('<?php echo base_url('cms_popup/capital_account'); ?>', 'account',700,800)" title="계정과목 관리">
                    ...
                    <tbody>
                        <?php foreach ($cb_list as $lt) : ?>
                            ...
                            if ($lt->inc == 0) $inc = '-';
                            else $inc = number_format($lt->inc);
                            if ($lt->exp == 0) $exp = '-';
                            else $exp = number_format($lt->exp);
                            if ($lt->acc) {
                                $acc = $lt->acc;
                            } else {
                                $acc = "-";
                            ...
                            <tr>
                                ...
                                <td class="">
                                    <div data-toggle="tooltip" data-placement="right" title="<?php echo $lt->cont . $memo; ?>" style="cursor: pointer;"><?php echo cut_string($lt->cont, 15, '..'); ?></div>
                                </td>
                                <td class="">
                                    <div data-toggle="tooltip" data-placement="right" title="<?php echo $lt->acc . $memo; ?>" style="cursor: pointer;"><?php echo cut_string($acc, 7, '..'); ?>
                                </td>
                                ...
                                <?php if ($auth12 > 1) : ?>
                                    ...
                                    <td class="center">
                                        <?php if ($m_auth->mem_is_admin === '1' or $m_auth->mem_level >= 60 or ($m_auth->mem_level < 60 && date('Y-m-d', strtotime('-3 day')) <= $lt->deal_date)) : //관리자와 마스터 쓰기권한이 아니면 최근 3일전 건에 대해서만 수정 가능   
                                            ?>
                                            <a href='javascript:' class="btn btn-info btn-xs" onclick="popUp_size('<?php echo base_url('/cms_popup/Capital_cash_book/cash_book/' . $lt->seq_num); ?>','cash_book','500','670')">
                                            <?php else : ?>
                                    ...
                                    <td class="center">
                                        <?php if ($m_auth->mem_is_admin === '1' or $m_auth->mem_level >= 60 or ($m_auth->mem_level < 60 && date('Y-m-d', strtotime('-3 day')) <= $lt->deal_date)) : //관리자와 마스터 쓰기권한이 아니면 최근 3일전 건에 대해서만 삭제 가능   
                                            ?>
                                            <a href='javascript:' class="btn btn-danger btn-xs" onclick="to_del(<?php echo $lt->seq_num . ', ' . $this->input->get('com_sel'); ?>);">
                                            <?php else : ?>
